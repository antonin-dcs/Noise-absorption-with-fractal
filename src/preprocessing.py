"""Preprocessing helpers for domain geometry, boundary labels, PDE coefficients,
and initial chi fields used by the solver and optimizer.
"""
...
def set_chi_partitioned(M, N, x, y, K=3,param=5):
    ...
    Parameters:
        M, N : int
            Dimensions de la matrice chi.
        x, y : list ou array
            Coordonnées du domaine de Robin.
        K : int
            Nombre de partitions à appliquer.
    ...
    # Découpage en K intervalles
    for k_part in range(K):
        k_begin = k_part * n_points // K
        k_end = (k_part + 1) * n_points // K
        # Ici on peut garder une portion centrale dans chaque partition
        sub_k_begin = k_begin + (k_end - k_begin) // param
        sub_k_end = min(k_begin + 3 * (k_end - k_begin) // param, len(x)-1)

        for k in range(sub_k_begin, sub_k_end):
            chi[int(y[k]), int(x[k])] = val
